"""
═══════════════════════════════════════════════════════════════════════════════
 ALGO RADAR v5 — UNIFIED STREAMLIT + FASTAPI WEBSOCKET TERMINAL
 NIFTY 50 & BANKNIFTY Real-Time Option Trading Predictor & Execution Engine
═══════════════════════════════════════════════════════════════════════════════
 Features:
  - Runs FastAPI + Async WebSocket server in background daemon thread inside app.py
  - Renders luxury Apple/Bloomberg Dark Terminal with TradingView Lightweight Charts v4.x
  - Zero-flicker 2-second live WebSocket streaming directly in Streamlit
...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  MODULE 3: NEWS SENTIMENT ENGINE                                         ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
def _get_finbert():
    ...
    try:
        ...
        _finbert = hf_pipeline("sentiment-analysis", model=mdl, tokenizer=tok, device=-1, top_k=None, truncation=True, max_length=512)
        return _finbert
    except Exception:
        return None
...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  MODULE 5: FASTAPI WEBSOCKET ENDPOINTS                                  ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
<main class="flex-1 max-w-7xl w-full mx-auto p-3 sm:p-6 space-y-4">
  ...
  <div class="grid grid-cols-1 lg:grid-cols-12 gap-4">
    ...
    <!-- LEFT/CENTER COLUMN (7 cols): HERO CARD & CANDLESTICK CHART -->
    ...
    <!-- RIGHT COLUMN (5 cols): AI CONSENSUS, STRIKE LADDER, PCR & SENTIMENT -->
...
function connectWS() {
  ...
  const port = (location.port && location.port !== '8501') ? location.port : '8000';
...
# ╔══════════════════════════════════════════════════════════════════════════╗
# ║  MODULE 7: STREAMLIT EMBEDDING ENGINE & BACKGROUND SERVER                ║
# ╚══════════════════════════════════════════════════════════════════════════╝
...
# Start background FastAPI WebSocket thread once
if "fastapi_server_started" not in st.session_state:
    st.session_state["fastapi_server_started"] = True
    t = threading.Thread(target=_run_fastapi_server, daemon=True)
    t.start()
